# Compare FULL, UNIF, LowCon, and CORE w.r.t. MSE and PMSE versus increasing r
rm(list = ls())
gc()
...
for (func_type in 1:6) {  
  # `function type in 7:8` is for high-dimensional cases
  ...
  for (i in 1:nloop) {
    ...
    if (func_type <= 6) {
      X_t <- grid_points(n_test, p) 
    } else {
      X_t <- matrix(runif(p * n_test), ncol = p)
    }
    ...
    generate_kernels <- function(x, x_t, k, n, n_test) {
      kg <- kernel_generate(x, k, id = 1:n, type_bs = "cr")
      kg_t <- kernel_generate(x_t, k, id = 1:n_test, type_bs = "cr")
    ...
    D_list <- list()
    if (func_type <= 4) {
      kernels <- generate_kernels(X[,1], X_t[,1], k, n, n_test)
      XX <- kernels$X
      XX_t <- kernels$X_t
      D_list <- list(D1 = kernels$D)
    } else if (func_type == 5) {
      kg1 <- kernel_generate_tp(X[,1], X[,2], k_ti, id = 1:n, type_bs = "te")
      kg1_t <- kernel_generate_tp(X_t[,1], X_t[,2], k_ti, id = 1:n_test, type_bs = "te")
      XX <- kg1$X
      XX_t <- kg1_t$X
      D_list <- list(D1 = kg1$D1, D2 = kg1$D2)
    } else {
      for (ii in 1:ncol(X)) {
        kernels <- generate_kernels(X[,ii], X_t[,ii], k, n, n_test)
        XX <- cbind(XX, kernels$X)
        XX_t <- cbind(XX_t, kernels$X_t)
    ...
    if (use_bam == FALSE) {
      ...
      df_test <- as.data.frame(cbind(y_t, X_t)) 
      if (func_type == 5) {
        colnames(df) <- colnames(df_test) <- c("y", "x1", "x2")
        fit <- bam(y ~ te(x1, x2, k = k_ti, bs = "ps"), 
                   data = df, method = "GCV.Cp")
      } else {
        set_colnames_and_formula <- function(func_type, p, k) {
          colnames <- c("y", paste0("x", 1:p))
          formula_x <- paste0("s(x", 1:p, ", k = ", k, ", bs = \"cr\")", collapse = "+")
          formula <- as.formula(paste("y ~", formula_x))
          list(colnames = colnames, formula = formula)
        }
        result <- set_colnames_and_formula(func_type, p, k)
        colnames(df) <- colnames(df_test) <- result$colnames
        fit <- bam(result$formula, data = df, method = "GCV.Cp")
      } 
